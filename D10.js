...
/* ESERCIZIO C
  Crea una variabile chiamata "me" e assegnaci un oggetto contenente
   le seguenti proprietà: name = il tuo nome, surname = il tuo cognome, age = la tua età.
*/
...
function dice() {
  let numeriCaso = Math.floor(Math.random() * 6) +1;
...
let esercizio2 = whoIsBigger(1,5);
...
let esercizio3 = splitMe('I love coding');
...
let esercizio4true = deleteOne ('hello', 1);
let esercizio4false = deleteOne ('ciao', 0);

console.log('è true :',  esercizio4true);
console.log('è false :',  esercizio4false);


/* ESERCIZIO 5
  Crea una funzione chiamata "onlyLetters" che riceve una stringa come parametro e la 
  ritorna eliminando tutte le cifre numeriche.
...
console.log(onlyLetters('i have 4 dogs e 5 gatti')) 
...
console.log(isThisAnEmail('ciao@umberto.it'))
console.log(isThisAnEmail('ciao-umberto.it'))
...
function whatDayIsIt() {
  ...
  return settimana[giornosettimana];
}

giornocorrente = whatDayIsIt();
...
let mioCompleanno = new Date(oggi.getFullYear(), 3, 28)
// console.log(mioCompleanno) 

let siIlMioCompleanno = oggi.getDate() === 28 & oggi.getMonth() === 3 ;


if (siIlMioCompleanno) {
...
/* ESERCIZIO 11
  ...
  parametri; deve ritornare l'oggetto fornito dopo aver eliminato
...
function deleteProp (ogg,str11) {

if (str11 === oggettoEs11.cognome ) {
  delete oggettoEs11.cognome ;
  return ogg
} else {
  return 'la stringa è differente'
}

}

console.log(deleteProp(oggettoEs11,strEs11))

// PER IL PROF = ESERCIZI IN BASSO DOPO L'ARRAY MOVIES
...
/* ESERCIZIO 19
  ...
  parametro e ritorna l'array "movies" fornito privo dell'elemento nella posizione 
...
let elementiTD = document.querySelectorAll('td');
...
/* ESERCIZIO 22
  Scrivi una funzione che, tramite un ciclo, stampa in console il testo contenuto in ogni
   tag <td> all'interno della pagina.
*/
...
stampa.forEach(ele => console.log(ele.textContent))
...
nuovoEle.innerText = 'nuovo';

  let miaLista = document.getElementById('myList'); // by id non serve # ricorda

miaLista.appendChild(nuovoEle);
...
function svuotaMiaLista () {

  let svuoto = document.querySelector('#myList'); // con #

  while(svuoto.firstChild) {

    svuoto.removeChild(svuoto.firstChild)
  }
...
function aggiungiCss () {




let csTest = document.querySelectorAll('tr')
...
movies.forEach(val => anniUscita.push(val.Year));
...
const onlyInLastMillennium = (movies) => {
  ...
  return movies.filter(val => parseInt(val.Year) <= 2000);
